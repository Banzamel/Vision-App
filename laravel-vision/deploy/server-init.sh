...
echo "  3. Add the Laravel scheduler cron — through the HestiaCP panel, NOT 'crontab -e'."
echo "     Hestia regenerates the user crontab from its own database, so hand-added"
echo "     entries disappear at the next v-rebuild-cron-jobs."
echo "       CRON -> ADD CRON JOB, every field '*', command:"
echo "       /usr/bin/php8.4 $LARAVEL_ROOT/artisan schedule:run >> /dev/null 2>&1"
echo "     Verify: crontab -l | grep schedule:run"
echo "     Drives vision:albums:sync (everyFiveMinutes) from routes/console.php."
